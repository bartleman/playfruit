...
// live-mode prefill lowered to ~200ms standing fill with a 250ms cap on the
// initial wait, and the live channel is drained eagerly instead of only below
// a 40% buffer threshold. See the Playfruit README "Provenance" section.
...
impl AudioStreamer {
    ...
    pub async fn start_live(&mut self, live_decoder: LiveAudioDecoder) -> Result<()> {
        ...
        // Live sources only start producing after this call returns, so waiting
        // here is pure startup/reconnect delay; the caller pre-charges its own
        // standing fill instead.
        let max_wait = std::time::Duration::from_millis(250);
        // Low-latency live mode: the standing fill accumulated here persists for
        // the whole session and adds directly to end-to-end latency. 10% of the
        // 2000ms buffer = ~200ms — enough to ride out scheduler hiccups without
        // pushing audible delay past lip-sync tolerance.
        let target_fill_pct = 10.0;
