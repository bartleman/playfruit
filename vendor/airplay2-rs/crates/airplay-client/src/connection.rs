...
// send_feedback() now propagates timeouts as Err instead of masking them as
// Ok, so supervisors can detect dead sessions. See the Playfruit README.
    ...
    pub async fn send_feedback(&mut self) -> Result<()> {
        ...
        match tokio::time::timeout(std::time::Duration::from_secs(2), self.rtsp.send(req)).await {
            ...
            Err(_) => {
                ...
                // Propagate: masking timeouts as Ok makes a dead session look
                // healthy forever. Callers that want leniency can tolerate N
                // consecutive failures before declaring the session dead.
                Err(airplay_core::error::Error::Timeout)
            }
        }
    }
